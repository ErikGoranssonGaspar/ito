...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.ITO_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ITO_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ITO_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.ITO_RELAY_URL).toBeUndefined();
    expect(env.VITE_ITO_RELAY_URL).toBeUndefined();
    expect(env.ITO_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ITO_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ITO_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.ITO_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ITO_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ITO_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "ITO_CLERK_PUBLISHABLE_KEY=pk_root\nITO_CLERK_JWT_TEMPLATE=template_root\nITO_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nITO_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "ITO_CLERK_PUBLISHABLE_KEY=pk_local\nITO_CLERK_JWT_TEMPLATE=template_local\nITO_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nITO_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).ITO_RELAY_URL).toBe("https://local.example.test");
    expect(
      loadRepoEnv({
        baseEnv: {
          ITO_CLERK_PUBLISHABLE_KEY: "pk_ci",
          ITO_CLERK_JWT_TEMPLATE: "template_ci",
          ITO_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          ITO_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      ITO_RELAY_URL: "https://ci.example.test",
      VITE_ITO_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        ITO_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_ITO_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ITO_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ITO_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          ITO_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ITO_RELAY_URL: "https://relay.example.test",
          ITO_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ITO_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          ITO_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      ITO_RELAY_URL: "https://relay.example.test",
      VITE_ITO_RELAY_URL: "https://relay.example.test",
      ITO_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      ITO_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      ITO_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ito-public-config-"));
