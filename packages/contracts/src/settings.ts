...
/**
 * The environment's theme, set with `ito theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `ito theme set` checks that an
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-ito/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Itô spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `ito` server (and with it every
  ...
  /**
   * Whether this server may install and run Ito's device helper processes.
   ...
   * of the same value from one they already applied: `ito theme set` must act
