...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@ito/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "ito-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Itô re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# ITO_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${ITO_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$ITO_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Ito ssh-askpass invoked without ITO_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Itô re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if ITO_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:ITO_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:ITO_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Ito ssh-askpass invoked without ITO_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "ito-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { ITO_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "ito" }),
  };
});
