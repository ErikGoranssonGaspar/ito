...
import { buildRemoteStopScript, buildRemoteItoRunnerScript } from "./tunnel.ts";
  ...
  () => {
    it.live("keeps the server PID and graceful shutdown through the node-script runner", () =>
      Effect.gen(function* () {
        ...
        const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "ito-runner-" });
...
server.listen(Number(process.env.ITO_TEST_PORT ?? 0), "127.0.0.1", () => {
          ...
          Effect.gen(function* () {
            const child = yield* spawner.spawn(
              ChildProcess.make("/bin/sh", ["-s", "--", "serve", "a path with spaces"], {
                ...
                env: {
                  PATH: bin,
                  ITO_TEST_PORT: String(port),
                },
                detached: false,
                stdin: Stream.make(
                  new TextEncoder().encode(buildRemoteItoRunnerScript({ nodeScriptPath: cliPath })),
                ),
              }),
            );
    ...
    it.live.each(["graceful", "timeout", "external"] as const)(
      ...
      (mode) =>
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
          const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "ito-stop-" });
          ...
          const runStop = Effect.fn("test.remoteStop")(function* () {
            const stop = yield* spawner.spawn(
              ChildProcess.make("/bin/sh", ["-s"], {
                cwd: fixture,
                env: { ITO_TEST_STATE_DIR: fixture },
