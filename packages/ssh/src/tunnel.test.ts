...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    const script = buildRemoteItoRunnerScript(ARCHIVE);

    assert.include(script, "ITO_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(script, "ITO_NODE_SCRIPT_PATH=''");
    assert.include(
      script,
      "ITO_RELEASE_BASE_URL='https://github.com/pingdotgg/ito/releases/download'",
    );
    assert.include(script, 'ITO_RUNTIME_DIR="$HOME/.ito/runtime/versions/$ITO_ARCHIVE_VERSION"');
    assert.include(script, 'ITO_ARCHIVE="ito-$ITO_ARCHIVE_VERSION-$ITO_PLATFORM-$ITO_ARCH.tar.gz"');
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$ITO_RUNTIME_DIR/ito" "$@"');
    assert.notInclude(script, "npx");
    assert.notInclude(script, "npm exec");
    assert.notInclude(script, "ito@latest");
    assert.notInclude(script, 'exec ito "$@"');
    ...
    assert.include(script, 'while ! mkdir "$ITO_LOCK" 2>/dev/null; do');
    assert.include(script, 'mv "$ITO_LOCK/pid.tmp" "$ITO_LOCK/pid"');
    assert.include(script, 'if ! kill -0 "$ITO_LOCK_OWNER" 2>/dev/null; then');
    assert.include(script, 'if [ "$ITO_LOCK_UNOWNED" -ge 5 ]; then');
    assert.include(script, 'if [ "$ITO_LOCK_WAITED" -ge 360 ]; then');
    assert.include(script, '"$ITO_STAGING/SHA256SUMS" 30');
    assert.include(script, '"$ITO_STAGING/$ITO_ARCHIVE" 240');
    assert.notInclude(script, "ITO_LOCK_CANDIDATE");
    assert.notInclude(script, "-mmin");
    assert.equal(script.split("if ! ito_runtime_ready; then").length - 1, 2);
    assert.isBelow(
      script.indexOf('"$ITO_STAGING/ito" --version'),
      script.indexOf('> "$ITO_STAGING/.install-complete"'),
    );
    ...
    assert.isBelow(
      ...
      script.indexOf('exec node "$ITO_NODE_SCRIPT_PATH" "$@"'),
    );
    assert.isBelow(
      script.indexOf('exec node "$ITO_NODE_SCRIPT_PATH" "$@"'),
      script.indexOf("ITO_ARCHIVE_VERSION="),
    );

    const launch = buildRemoteLaunchScript({
      ...ARCHIVE,
      releaseBaseUrl: "https://mirror.example/ito/",
    });
    assert.include(launch, "ITO_ARCHIVE_MODE=1");
    assert.include(launch, "ITO_RELEASE_BASE_URL='https://mirror.example/ito'");
    ...
    assert.include(buildRemoteLaunchScript(NODE_SCRIPT), "ITO_ARCHIVE_MODE=0");
  });
  ...
  it("rejects archive versions that are not a single exact version segment", () => {
    for (const archiveVersion of [
      ...
      assert.throws(
        () => buildRemoteItoRunnerScript({ archiveVersion }),
        SshInvalidArchiveVersionError,
        undefined,
        archiveVersion,
      );
    ...
    assert.include(
      buildRemoteItoRunnerScript(ARCHIVE),
      "ITO_ARCHIVE_VERSION='1.2.3-preview.20260911.4'",
    );
  });
  ...
  it("refuses to build a runner with neither an archive version nor a node script", () => {
    for (const input of [undefined, {}, { archiveVersion: "  " }, { nodeScriptPath: null }]) {
      assert.throws(() => buildRemoteItoRunnerScript(input), SshMissingRunnerError);
    }
    assert.throws(() => buildRemoteLaunchScript(), SshMissingRunnerError);
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteItoRunnerScript(NODE_SCRIPT);

    assert.include(script, "ITO_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("builds the remote ito runner with a node script override", () => {
    const script = buildRemoteItoRunnerScript({
      ...NODE_SCRIPT,
    ...
    assert.include(
      script,
      "ITO_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$ITO_NODE_SCRIPT_PATH" "$@"');
    assert.include(script, "ITO_ARCHIVE_VERSION=''");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `ITO_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for ITO_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
    assert.notInclude(script, "npx");
  });

  it("uses the remote ito runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(devLaunch, `ITO_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    // The generated script stays ASCII: the remote shell's locale is not ours.
    assert.include(launch, "Remote Ito server did not become ready");
    ...
    assert.include(launch, "ITO_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(
          ...
          if (args.includes("sh")) {
            ...
            if (mode === "failed stop" && stopCommandCount === 1) {
              return {
                ...
                stderr: Stream.make(
                  new TextEncoder().encode("Remote Itô server did not stop within 2 seconds.\n"),
                ),
              };
            }
        ...
        if (mode === "failed stop") {
          ...
          if (Result.isFailure(disconnected)) {
            ...
            assert.equal(
              ...
              "Remote Itô server did not stop within 2 seconds.",
            );
          }
        } else {
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "ito-archive-runner-" });
        const releaseBaseUrl = yield* makeMirror(root);
        const runner = `${root}/run-ito.sh`;
        yield* fs.writeFileString(
          runner,
          buildRemoteItoRunnerScript({ archiveVersion, releaseBaseUrl }),
        );
        ...
        for (const result of results) {
          ...
          assert.include(result.stdout, `ito v${archiveVersion}`);
        }
        const versionsDir = `${home}/.ito/runtime/versions`;
