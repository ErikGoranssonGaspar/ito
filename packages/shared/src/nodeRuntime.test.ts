...
describe("Node runtime selection", () => {
  it.effect("keeps the current Node or Electron runtime without requiring Node on PATH", () =>
    Effect.gen(function* () {
      for (const executable of ["/runtime/node", "/Applications/Itô.app/Electron"]) {
        expect(
  ...
  it.effect("uses installed Node instead of the standalone Itô executable", () =>
    Effect.gen(function* () {
  ...
  it.effect("rejects a hard-linked node alias pointing back at the standalone app", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped();
      const platform = yield* HostProcessPlatform;
      const executable = path.join(directory, platform === "win32" ? "ito.exe" : "ito");
      const node = path.join(directory, platform === "win32" ? "node.exe" : "node");
