import { COMPOSER_CONTEXT_CLIPBOARD_MIME, ComposerContextClipboardFragment } from "@ito/contracts";
...
export function encodeComposerContextClipboardHtml(
  ...
  if (html !== undefined)
    return `<div data-ito-context-fragment="${encodeURIComponent(fragment)}">${html}</div>`;
  const escaped = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  return `<pre data-ito-context-fragment="${encodeURIComponent(fragment)}">${escaped}</pre>`;
}
...
export function decodeComposerContextClipboardHtml(
  ...
  const encoded = /data-ito-context-fragment=["']([^"']+)["']/.exec(html)?.[1];
