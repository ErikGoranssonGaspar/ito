...
import { resolveGitWorktreePath, resolveWorktreeItoHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ito-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeItoHome", () => {
  it.effect("answers with .ito before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeItoHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".ito"));
