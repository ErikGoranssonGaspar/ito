    ...
    it.effect("flushes buffered trace records on close", () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-trace-sink-" });
  ...
  it.each([
    {
      ...
      input: "authorization=Basic%20abc%3D%3D,x-tenant=ito",
      expected: { authorization: "Basic abc==", "x-tenant": "ito" },
    },
    {
