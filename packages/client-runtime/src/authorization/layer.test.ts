import { AuthStandardClientScopes, EnvironmentId } from "@ito/contracts";
...
const makeHarness = Effect.fn("TestRemoteAuthorization.makeHarness")(function* (input: {
  ...
  const layer = RemoteEnvironmentAuthorization.layer.pipe(
    Layer.provide(Layer.mergeAll(presentation, remoteHttpClientLayer(fetch.fetchFn))),
  );
  return { fetch, layer };
});
...
describe("RemoteEnvironmentAuthorization", () => {
  it.effect("reuses a validated bearer descriptor while issuing fresh websocket tickets", () =>
    Effect.gen(function* () {
      ...
      expect(
        harness.fetch.calls.filter(([url]) => String(url).endsWith("/.well-known/ito/environment")),
      ).toHaveLength(1);
      expect(
