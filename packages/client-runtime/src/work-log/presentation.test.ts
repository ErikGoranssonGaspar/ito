...
import { ThreadId } from "@ito/contracts";
...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__ito__preview_click",
    "mcp__ito__preview_click",
    "mcp__ito__preview_click",
    "Ito.preview_click",
    "ito · preview_click completed",
    "ito/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__ito__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "ito · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "ito", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "Ito.preview_click" };
  ...
  it.each([
    ...
    ["ito_thread_read", "Reading an Itô thread", "Read an Itô thread"],
    ["ito_thread_send", "Sending to an Itô thread", "Sent to an Itô thread"],
    [
      "ito_worktree_handoff",
  ...
  it("keeps Itô branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__ito__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "ito" });
  });
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "ito", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "ito", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__ito__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves ito attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.ito/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__ito__link_pull_request",
    "mcp__ito__link_pull_request",
    "Ito · link_pull_request",
    "ito/link_pull_request",
    "link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__ito__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "Ito · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "ito", name: "Itô", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "Ito · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "Ito · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__ito__device_screenshot" },
  ...
  it("does not classify another server's tools as Itô device controls", () => {
    expect(
      summarizeToolGroup([
        {
          ...deviceEntry("device_open"),
