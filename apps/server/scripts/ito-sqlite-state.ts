...
import { fromJsonStringPretty } from "@ito/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Ito once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.ito database. Use an isolated --base-dir.";
  }
}
...
const itoSqliteStateCommand = Command.make(
  "ito-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Itô base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Itô SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(itoSqliteStateCommand, { version: "0.0.0" }).pipe(
