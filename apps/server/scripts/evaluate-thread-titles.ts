...
import { CodexSettings, ProviderInstanceId } from "@ito/contracts";
...
await Effect.runPromise(
  Effect.gen(function* () {
    ...
    const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ito-title-evaluation-" });
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), { prefix: "ito-title-evaluation-state-" }),
        ),
