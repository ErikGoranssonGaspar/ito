...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allIto: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allIto = input.counters.allIto;
  for (const processDelta of input.deltas) {
    ...
    allIto = incrementCounters(allIto, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allIto = incrementCounters(allIto, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allIto = incrementCounters(allIto, { processExits: 1 });
  }

  return { backend, electron, monitor, allIto };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allIto: aggregate(ordered, counters.allIto),
    },
    deltas: processDeltas,
  };
}
