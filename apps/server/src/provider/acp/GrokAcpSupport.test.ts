...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Itô runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Itô is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Itô referrer through Grok OAuth env", () => {
