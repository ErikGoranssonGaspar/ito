...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@ito/contracts";
...
/**
 * Client captured the plan for Ito's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Ito UI.
 */
 ...
 * Used so Ito can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `ito-xai-prompt-${nextPromptFallbackId}`;
    });
