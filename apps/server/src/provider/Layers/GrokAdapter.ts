...
import { getModelSelectionStringOptionValue } from "@ito/shared/model";
import { stableStringify } from "@ito/shared/stableJson";
...
export function selectGrokPermissionOptionId(
  ...
  // Grok 4.6 often omits allow_always. Ito still offers "Always allow this session".
...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    /** Surface Grok plan.md as Ito's proposed-plan card (while writing + on exit). */
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            clientInfo: { name: "ito", version: "0.0.0" },
            ...(mcpSession
              ? {
                  mcpServers: [
                    {
          ...
          const started = yield* Effect.gen(function* () {
            ...
            // Capture plan into Ito proposed-plan UI and abandon the native gate so
      ...
      Effect.gen(function* () {
        if (/^\/always-approve(?:\s|$)/i.test(input.input?.trim() ?? "")) {
          return yield* new ProviderAdapterRequestError({
            ...
            detail: "Change permissions with Itô's permission selector instead of /always-approve.",
          });
        }
