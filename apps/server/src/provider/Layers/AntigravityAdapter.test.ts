...
const layer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ito-antigravity-adapter-test-",
...
it.layer(layer)("AntigravityAdapter", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "ito-antigravity-transport-",
        });
          ...
          makeRuntime: (input) =>
            makeAntigravityAcpRuntime({
              ...
              spawn: {
                ...
                env: {
                  ...process.env,
                  ITO_ACP_ANTIGRAVITY: "1",
                  ITO_ACP_REQUEST_LOG_PATH: requestLog,
                },
                extendEnv: false,
              },
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ito-agy-fs-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "ito-agy-outside-" });
