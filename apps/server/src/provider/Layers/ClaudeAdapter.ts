...
import { parseCliArgs } from "@ito/shared/cliArgs";
...
} from "@ito/shared/claudeCompaction";
import { HostProcessIsExecutable } from "@ito/shared/hostProcess";
...
// conversation messages from the truncated end, then remap Ito turn starts.
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "model_refusal_fallback":
        ...
        return;
      // Inner protocol/UX details with no Ito surface today — consumed
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    // Wire-only command bookkeeping has no user-facing Ito lifecycle.
    ...
    switch (message.type) {
      ...
      case "rate_limit_event":
        ...
        return;
      // Composer prompt suggestions have no Ito surface; consumed deliberately.
      // `conversation_reset` announces a CLI-side conversation id swap
      // (e.g. /clear); Ito keeps its own thread identity and resume cursor.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See https://github.com/pingdotgg/ito/issues/2388
      ...
      const handleResumeDialog = Effect.fn("handleResumeDialog")(function* (
        ...
        // The question copy lives in @ito/shared/claudeCompaction because
      ...
      // A permission launch arg is folded into the mode Ito sends rather than
  ...
  const rollbackThread: ClaudeAdapterShape["rollbackThread"] = Effect.fn("rollbackThread")(
    function* (threadId, numTurns) {
      ...
      // their Ito turn count agrees; steers must never be treated as extra turns.
      if (
