...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "ito-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%ITO_TEST_NODE_BINARY%" "%ITO_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$ITO_TEST_NODE_BINARY" "$ITO_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          ITO_TEST_NODE_BINARY: executablePath,
          ITO_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
