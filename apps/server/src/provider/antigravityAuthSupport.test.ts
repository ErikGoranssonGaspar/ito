...
import { ProviderInstanceId } from "@ito/contracts";
import {
  HostProcessExecutablePath,
  HostProcessIsExecutable,
  HostProcessPlatform,
} from "@ito/shared/hostProcess";
...
describe("Antigravity process environment", () => {
  const profile: AntigravityProfile = {
    platform: "linux",
    geminiHome: "/ito/userdata/providers/antigravity/profile",
    acpDirectory: "/ito/userdata/providers/antigravity/profile/antigravity-acp",
    tokenPath: "/ito/userdata/providers/antigravity/profile/antigravity-acp/acp_token.json",
    tempDirectory: "/ito/userdata/providers/antigravity/profile/antigravity-acp/tmp",
...
it.layer(NodeServices.layer)("Antigravity profile preparation", (it) => {
  it.effect("runs the browser helper with installed Node in standalone builds", () =>
    Effect.gen(function* () {
      ...
      expect(profile.browserCommand).not.toContain("/packaged/ito");
      expect(yield* fs.exists(profile.acpDirectory)).toBe(true);
    }).pipe(
      Effect.provideService(HostProcessIsExecutable, true),
      Effect.provideService(HostProcessExecutablePath, "/packaged/ito"),
    ),
  );
