...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "ito-cursor-driver-copy-command-",
}).pipe(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "ito-cursor-driver-" });
    ...
    Effect.gen(function* () {
      const instance = yield* CursorDriver.create({
        ...
        config: {
          ...
          binaryPath: NodePath.join(NodeOS.tmpdir(), "ito-cursor-missing", "cursor-agent"),
        },
      });
