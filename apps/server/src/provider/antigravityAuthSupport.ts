...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@ito/contracts";
import { HostProcessPlatform } from "@ito/shared/hostProcess";
import { resolveNodeExecutable, nodeRuntimeUnavailableMessage } from "@ito/shared/nodeRuntime";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__Ito_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/ito-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by Ito. */
...
function antigravityEnvironment(
  ...
  // Point it at a Ito-owned directory so the driver can reclaim the space.
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which Ito points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The Itô runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and Ito browser-helper URLs without logging stderr. */
