...
import { CheckpointRef, GitCommandError } from "@ito/contracts";
...
const makeCheckpointFixture = Effect.fn("makeCheckpointFixture")(function* (
  ...
  const checkpointRef = CheckpointRef.make("refs/ito/checkpoints/test");
...
it.effect("checkpoint capture does not rerun clean filters for unchanged indexed files", () =>
  Effect.gen(function* () {
    ...
    const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-checkpoint-cache-" });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const driver = yield* GitVcsDriver.makeVcsDriverShape();
          const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ito-checkpoint-sparse-" });
          ...
          for (const turn of [1, 2]) {
            const ref = CheckpointRef.make(`refs/ito/checkpoints/sparse/${turn}`);
            if (turn === 2) {
...
it.effect("checkpoint capture keeps the legacy path when Git lacks add --sparse", () =>
  Effect.gen(function* () {
    ...
    const driver = yield* GitVcsDriver.makeVcsDriverShape();
    const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ito-checkpoint-legacy-" });
    const { git, checkpointRef } = yield* makeCheckpointFixture(driver, cwd);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const liveProcess = yield* VcsProcess.VcsProcess;
        const driver = yield* GitVcsDriver.makeVcsDriverShape();
        const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "ito-checkpoint-inspection-" });
        const { git, checkpointRef } = yield* makeCheckpointFixture(driver, cwd);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const driver = yield* GitVcsDriver.makeVcsDriverShape();
        const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-checkpoint-racy-" });
        const { git, checkpointRef } = yield* makeCheckpointFixture(driver, cwd);
...
it.effect("checkpoint capture preserves racy edits made after resetting the index", () =>
  Effect.gen(function* () {
    ...
    const liveProcess = yield* VcsProcess.VcsProcess;
    const driver = yield* GitVcsDriver.makeVcsDriverShape();
    const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-checkpoint-racy-reset-" });
    const { git, checkpointRef } = yield* makeCheckpointFixture(driver, cwd);
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const captureCwd = nested ? path.join(cwd, "scope") : cwd;
          const first = CheckpointRef.make("refs/ito/checkpoints/turns/1");
          const second = CheckpointRef.make("refs/ito/checkpoints/turns/2");
...
for (const indexState of ["missing", "invalid"] as const) {
  it.effect(`checkpoint capture falls back when the user index is ${indexState}`, () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const driver = yield* GitVcsDriver.makeVcsDriverShape();
      const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-checkpoint-index-" });
      const { git, checkpointRef } = yield* makeCheckpointFixture(driver, cwd);
      const indexPath = path.join(cwd, ".git", "index");
...
it.effect("restores empty checkpoints without changing paths outside the workspace", () =>
  Effect.gen(function* () {
    ...
    const driver = yield* GitVcsDriver.makeVcsDriverShape();
    for (const nested of [false, true]) {
      const root = yield* fileSystem.makeTempDirectoryScoped({ prefix: "ito-empty-checkpoint-" });
      ...
      const checkpointRef = CheckpointRef.make("refs/ito/checkpoints/empty");
      yield* driver.checkpoints.captureCheckpoint({ cwd, checkpointRef });
      if (nested) {
...
it.effect("GitVcsDriver flushes checkpoint objects and refs to disk before publishing them", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* driver.checkpoints.captureCheckpoint({
      cwd: "/repo",
      checkpointRef: CheckpointRef.make("refs/ito/checkpoints/thread/turn/1"),
    });
    ...
    assert.deepStrictEqual(observedArgs.at(-1), [
      ...
      "update-ref",
      "refs/ito/checkpoints/thread/turn/1",
      "commit0000",
    ]);
  }).pipe(
    Effect.provide(
