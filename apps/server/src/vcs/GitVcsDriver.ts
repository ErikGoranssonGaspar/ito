...
export const makeVcsDriverShape = Effect.fn("makeGitVcsDriverShape")(function* () {
  ...
  // an unclean restart can leave 0-byte files under refs/ito/** that break every
  ...
  const checkpoints: VcsDriver.VcsCheckpointOps = {
    captureCheckpoint: Effect.fn("GitVcsDriver.checkpoints.captureCheckpoint")(function* (input) {
      ...
      const tempIndexPath = path.join(
        gitCommonDir,
        `ito-checkpoint-index-${NodeCrypto.randomUUID()}`,
      );
      const commitEnv: NodeJS.ProcessEnv = {
        ...process.env,
        GIT_INDEX_FILE: tempIndexPath,
        GIT_AUTHOR_NAME: "Itô",
        GIT_AUTHOR_EMAIL: "ito@users.noreply.github.com",
        GIT_COMMITTER_NAME: "Itô",
        GIT_COMMITTER_EMAIL: "ito@users.noreply.github.com",
      };
      ...
      yield* Effect.gen(function* () {
        ...
        const message = `ito checkpoint ref=${input.checkpointRef}`;
