...
import { sanitizeBranchFragment } from "@ito/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@ito/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("ITO_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("ITO_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("ITO_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("ITO_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@ito/server/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `ito/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set ITO_BITBUCKET_EMAIL and ITO_BITBUCKET_API_TOKEN, or ITO_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
