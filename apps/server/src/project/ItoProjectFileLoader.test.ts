...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(ItoProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "ito.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("ItoProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid ito.json", () =>
      Effect.gen(function* () {
        const loader = yield* ItoProjectFileLoader.ItoProjectFileLoader;
    ...
    it.effect("returns none when ito.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* ItoProjectFileLoader.ItoProjectFileLoader;
        const cwd = yield* makeTempDir;
