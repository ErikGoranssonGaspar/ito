...
import { normalizeProjectPathForComparison } from "@ito/shared/path";
...
>()("@ito/server/project/AgentSessionScanner") {}
...
/**
 * Itô runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.ito/worktrees` layout, which
 * also catches sandboxes from other Ito homes on the same machine. Separators
...
function isItoManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.ito/worktrees/")
  );
}
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isItoManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
