...
import { EnvironmentHttpApi } from "@ito/contracts";
...
).pipe(Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "ito-auth-http-test-" })));
    ...
    return yield* Effect.acquireUseRelease(
      ...
      ([environmentA, environmentB]) =>
        Effect.tryPromise(async () => {
          ...
          const devCookie = devCookies.find((cookie) => cookie.startsWith("ito_dev_session_"));
          expect(devCookie).toContain("HttpOnly");
          expect(devCookie).toContain(`=${DEV_TOKEN};`);
          expect(devCookies).toContainEqual(
            expect.stringMatching(/^ito_session_[^=]*=;.*Max-Age=0/),
          );
          ...
          expect(restrictedCookies[0]).toMatch(/^ito_session_/);
          expect(restrictedCookies[0]).not.toContain("ito_dev_session_");
        }),
      ([environmentA, environmentB]) =>
