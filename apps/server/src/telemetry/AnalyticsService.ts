...
import type { ClientOs } from "@ito/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("ITO_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("ITO_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("ITO_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("ITO_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("ITO_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
...
>()("@ito/server/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          itoCodeVersion: packageJson.version,
          clientType,
