...
const remoteDebuggingPort = process.env.ITO_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line ito/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.ITO_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.ITO_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--ito-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
