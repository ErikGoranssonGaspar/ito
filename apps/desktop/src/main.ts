...
import type { RemoteItoRunnerOptions } from "@ito/ssh/tunnel";
...
): RemoteItoRunnerOptions => {
  const devRemoteEntryPath = Option.getOrUndefined(environment.devRemoteItoServerEntryPath);
...
// Pool layer instantiates the backend factory once for the local backend
// and exposes it via pool.primary. Consumers go through the pool; the
// legacy DesktopBackendManager service is gone.
...
// Set userData and acquire the single-instance lock before Electron emits ready.
