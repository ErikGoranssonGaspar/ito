...
export type SnapShotAnimationDestination = {
  ...
  /** Unit coordinates in Ito's content area; GNOME supplies the real compositor origin. */
  ...
  private async showOverlays(
    ...
    try {
      ...
      if (this.waitForCompositorFrame) {
        // Cover the source with a composited snapshot before the caller reveals Ito.
