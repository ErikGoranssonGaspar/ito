import { EnvironmentId, MessageId, ThreadId, type AssistantCitation } from "@ito/contracts";
...
describe("splitPromptIntoComposerSegments", () => {
  ...
  it("keeps malformed citation links as editable text", () => {
    const prompt = "[Assistant quote](ito-citation://v1/env/thread/message?text=missing+metadata)";
  ...
  it("leaves a context link with an unparsable href as text", () => {
    const prompt = "see [x](ito-context://v1/terminal/ctx 1) now";
    expect(splitPromptIntoComposerSegments(prompt)).toEqual([{ type: "text", text: prompt }]);
  });
});
