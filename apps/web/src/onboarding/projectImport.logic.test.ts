import { EnvironmentId, ProjectId, type AgentSessionProjectCandidate } from "@ito/contracts";
...
describe("groupOnboardingProjects", () => {
  it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
    const main = candidate("/code/ito", {
      git: github("pingdotgg/ito"),
      threadCount: 79,
      lastActiveAt: "2026-08-21T12:00:00.000Z",
    });
    const clone = candidate("/code/clones/ito-2", {
      git: github("pingdotgg/ito"),
      threadCount: 13,
      lastActiveAt: "2026-08-10T12:00:00.000Z",
    });
    const older = candidate("/code/fleet", {
      git: github("ito/fleet"),
      threadCount: 295,
    ...
    expect(
      ...
      {
        label: "pingdotgg/ito",
        paths: ["/code/ito", "/code/clones/ito-2"],
        threadCount: 92,
        lastActiveAt: "2026-08-21T12:00:00.000Z",
      },
      {
