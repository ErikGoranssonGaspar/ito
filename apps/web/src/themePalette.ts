...
export const ITO_THEME_ID = "ito" as const;
export const ORCHID_THEME_ID = "orchid" as const;
...
export const CUSTOM_THEMES_STORAGE_KEY = "ito:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "ito:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "ito:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "ito:theme-halves:v1";

const LEGACY_ORCHID_THEME_ID = "t3-chat";
const LEGACY_ORCHID_DARK_THEME_ID = "t3-chat-dark";
...
// Earlier builds shipped every maintainer theme under a t3- prefix, and the
// flagship pink palette under the name of another product. Someone who chose
// that palette keeps it — it is Orchid now, not the app's own colours.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_ORCHID_THEME_ID]: ORCHID_THEME_ID,
  [LEGACY_ORCHID_DARK_THEME_ID]: ORCHID_THEME_ID,
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_ORCHID_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark pink palette as a separate theme. Keep those
// preferences readable while mapping them to the dark variant.
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_ORCHID_DARK_THEME_ID ? "dark" : null;
}

/**
 * The palette Itô wears with no theme installed, captured from the app's
 ...
 */
const ITO_CODE_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const ITO_CODE_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard Itô look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Itô palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(ITO_CODE_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(ITO_CODE_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Itô shows without a theme, read from the app's own
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? ITO_THEME.variants!.dark! : ITO_THEME.colors;
}
