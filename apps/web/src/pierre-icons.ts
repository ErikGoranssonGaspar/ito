...
import { VIDEO_FILE_EXTENSIONS } from "@ito/shared/video";
...
const PIERRE_ICON_SPRITE_ID = "ito-pierre-file-icon-sprite";

const ITO_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <!-- Lucide Film icon, ISC license. -->
  <symbol id="ito-file-icon-video" viewBox="0 0 24 24">
  ...
  <symbol id="ito-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="ito-file-icon-pnpm" viewBox="0 0 32 32">
...
export const ITO_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: ITO_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "file-tree-builtin-npm",
    "tsconfig.json": "file-tree-builtin-typescript",
    "agents.md": "ito-file-icon-agents",
    "pnpm-lock.yaml": "ito-file-icon-pnpm",
    "pnpm-workspace.yaml": "ito-file-icon-pnpm",
  },
  byFileExtension: Object.fromEntries(
    VIDEO_FILE_EXTENSIONS.map((extension) => [extension, "ito-file-icon-video"]),
  ),
} satisfies FileTreeIcons;

const completeIconResolver = createFileTreeIconResolver(ITO_PIERRE_ICONS);
...
export function ensurePierreIconSprite(): void {
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${ITO_FILE_ICON_SPRITE}`;
