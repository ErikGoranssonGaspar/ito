import { EnvironmentId, type ItoProjectFileScript } from "@ito/contracts";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@ito/client-runtime/state/runtime";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@ito/shared/keybindings";
import { ChevronDownIcon, PlusIcon } from "lucide-react";
import { useCallback, useMemo, useState } from "react";
import { useItoProjectFileState } from "../../hooks/useItoProjectFileScripts";
...
export function ProjectActionsSettings() {
  ...
  // A project's ito.json can declare actions to import. Read it from the
  ...
  const importableScripts = useMemo(
    ...
    [scripts, itoFile.scripts],
  );
  const importFileScript = useCallback(
    async (fileScript: ItoProjectFileScript) => {
  ...
  return (
    <SettingsSection id="project-actions" title="Actions">
            ...
            {importableScripts.length > 0 ? (
              <Menu>
                ...
                <MenuPopup align="end" className="w-72">
                  <MenuGroup>
                    <MenuGroupLabel>Import from ito.json</MenuGroupLabel>
      ...
      {itoFile.status === "invalid" ? (
        <SettingsRow
          title="ito.json is invalid"
          description="A ito.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
