import { EnvironmentId, ProviderInstanceId } from "@ito/contracts";
import {
  createMemoryHistory,
  createRootRoute,
  createRoute,
...
const checkoutSearch = {
  project: "repository:ito",
  machine: "remote-server",
  checkout: "remote-server:/home/user/Ito",
};
...
describe("settings scope navigation", () => {
  ...
  it.each(["/settings/projects", "/settings/integrations", "/settings/source-control"] as const)(
    ...
    async (to) => {
      ...
      const regroupedCheckout = { ...checkoutSearch, project: "separate:ito" };
      await router.navigate({
        from: "/settings",
