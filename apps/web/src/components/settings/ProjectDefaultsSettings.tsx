...
export function ProjectDefaultsSettings({ category }: { category: ProjectSettingsCategory }) {
  ...
  // A checkout's ito.json wins over the environment default when the project
  ...
  const repositoryEnvMode = itoFile.file?.defaultThreadEnvMode ?? null;
  const inheritedEnvModeLabel =
    workspaceSource === "project"
      ? null
      : repositoryEnvMode
        ? `${resolveEnvModeLabel(repositoryEnvMode)} (ito.json)`
        : null;
      ...
      {category === "general" ? (
        <>
          ...
          <SettingsRow
            ...
            description={
              isProjectScope
                ? "Where new threads in this project start. A ito.json preference applies when the project has no override."
                : "Where new threads start, unless overridden by the project or ito.json."
            }
            status={
