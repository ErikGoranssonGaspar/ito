...
export function ResourceTelemetryDiagnostics({
  ...
  const allIto = snapshot?.groups.allIto;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allIto ? `${allIto.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={allIto ? `${formatCpuTime(allIto.cpuTimeMs)} observed CPU time` : undefined}
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allIto ? formatBytes(allIto.currentRssBytes) : "..."}
              detail={
                allIto ? `${formatBytes(allIto.peakRssBytes)} combined process peaks` : undefined
              }
            />
            <IconStat
              icon={<ActivityIcon className="size-3.5" />}
              label="Process count"
              value={allIto ? String(allIto.processCount) : "..."}
              detail={
                allIto ? `${allIto.processStarts} starts · ${allIto.processExits} exits` : undefined
              }
            />
            <IconStat
              icon={<HardDriveIcon className="size-3.5" />}
              label="Read throughput"
              value={allIto ? formatRate(allIto.ioReadBytesPerSecond) : "..."}
              detail={allIto ? `${formatBytes(allIto.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allIto ? formatRate(allIto.ioWriteBytesPerSecond) : "..."}
              detail={allIto ? `${formatBytes(allIto.ioWriteBytes)} observed` : undefined}
              tone={
                allIto && allIto.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allIto && allIto.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Ito operations so process spikes can be correlated with specific
