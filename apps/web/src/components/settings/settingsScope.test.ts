...
const groups = [group("ito", [first, second, third]), group("other", [other])];

describe("settings scope search", () => {
  it("ignores the retired scope key from older links", () => {
    expect(validateSettingsScopeSearch({ scope: "device", project: "ito" })).toEqual({
      project: "ito",
    });
  ...
  it("retains legacy project and machine links without inventing an explicit broad scope", () => {
    expect(
      validateSettingsScopeSearch({ project: "ito", machine: laptopId, unused: true }),
    ).toEqual({
      project: "ito",
      machine: laptopId,
    });
...
describe("settings scope resolution", () => {
  ...
  it("keeps all physical members in a project aggregate, including several on one environment", () => {
    expect(resolveSettingsScope({ project: "ito" }, groups, environments)).toMatchObject({
      kind: "project",
      environmentId: null,
  ...
  it("preserves legacy project plus machine aggregates with multiple checkouts", () => {
    expect(
      resolveSettingsScope({ project: "ito", machine: laptopId }, groups, environments),
    ).toMatchObject({
      kind: "project",
  ...
  it("narrows a checkout target to exactly one member, deriving its environment when omitted", () => {
    expect(
      resolveSettingsScope(
        { project: "ito", checkout: second.physicalProjectKey },
        groups,
        environments,
      ),
    ).toMatchObject({
  ...
  it.each([
    { project: "missing" },
    { machine: "removed" },
    { project: "ito", machine: "removed" },
    { project: "ito", checkout: "deleted" },
    { project: "other", machine: laptopId },
    { project: "other", checkout: first.physicalProjectKey },
    { project: "ito", machine: serverId, checkout: first.physicalProjectKey },
  ...
  it("leaves a removed checkout unavailable while sibling checkouts remain", () => {
    ...
    expect(
      resolveSettingsScope(search, [group("ito", [second, third])], environments),
  ...
  it("does not select another environment after removing a project's last local checkout", () => {
    const search = { project: "ito", machine: laptopId };
    expect(resolveSettingsScope(search, groups, environments)).toMatchObject({
      kind: "project",
      members: [first, second],
    });
    expect(resolveSettingsScope(search, [group("ito", [third])], environments)).toMatchObject({
      kind: "unavailable",
      members: [],
      environmentIds: [],
    });
  ...
  it("rejects a cached checkout whose environment was removed", () => {
    expect(
      resolveSettingsScope(
        { project: "ito", checkout: third.physicalProjectKey },
        groups,
