...
describe("theme files", () => {
  ...
  it("merges a small user file onto the matching contrast-safe base palette", () => {
    ...
    expect(theme).toMatchObject({
      ...
      colors: {
        ...
        // Roles the file leaves out come straight from the flagship palette.
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...ORCHID_THEME,
      colors: { ...ORCHID_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: ORCHID_THEME.id,
      name: ORCHID_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(ORCHID_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(ORCHID_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(ORCHID_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(ORCHID_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(ORCHID_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(ORCHID_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Orchid palette faithful and readable", () => {
    expectThemeColors(ORCHID_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(ORCHID_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(ORCHID_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [ORCHID_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy t3-chat-dark preference to dark Orchid", () => {
    expect(getThemeDefinition("t3-chat-dark")).toBe(ORCHID_THEME);
  ...
  it("resolves legacy ito-prefixed ids onto the renamed themes", () => {
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", ORCHID_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = ORCHID_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(ORCHID_THEME)).toBe(null);
  });
});
