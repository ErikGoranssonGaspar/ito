...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Itô-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "ito-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "ito-file-icon-pnpm");
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(ITO_PIERRE_ICONS.byFileName).filter((name) => name.startsWith("ito-")),
    );
    for (const iconName of customIconNames) {
      assert.include(ITO_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
