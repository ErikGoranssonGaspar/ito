...
const THEME_STORAGE_KEY = "ito:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Orchid follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "orchid", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Orchid",
      storage: {
        [THEME_STORAGE_KEY]: "orchid",
    ...
    {
      name: "legacy t3-chat-dark resolves to dark Orchid",
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("orchid");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "ito:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("orchid");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(ORCHID_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "orchid",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "ito:theme-halves:v1": JSON.stringify({ dark: "t3-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "orchid",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "ito:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("orchid");
    expect(boot.isDark).toBe(true);
  });
