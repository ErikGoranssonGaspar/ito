...
import { resolveWorkEntryToolPresentation } from "@ito/client-runtime/work-log/presentation";
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "ito · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "ito · preview_status",
          toolSurface: "browser",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("ito · preview_status");
