import { ProjectId, ThreadId } from "@ito/contracts";
...
describe("uiStateStore pure functions", () => {
  ...
  it("stores the sidebar project scope and resets it to all projects", () => {
    const scoped = setSidebarProjectScopeKey(makeUiState(), "github.com/pingdotgg/ito");

    expect(scoped.sidebarProjectScopeKey).toBe("github.com/pingdotgg/ito");
    expect(setSidebarProjectScopeKey(scoped, "github.com/pingdotgg/ito")).toBe(scoped);
...
describe("uiStateStore persistence", () => {
  ...
  it("restores the sidebar project scope across reloads", () => {
    persistState(makeUiState({ sidebarProjectScopeKey: "github.com/pingdotgg/ito" }));

    const persisted = JSON.parse(
      localStorageStub.getItem(PERSISTED_STATE_KEY) ?? "{}",
    ) as PersistedUiState;

    expect(parsePersistedState(persisted).sidebarProjectScopeKey).toBe("github.com/pingdotgg/ito");
  });
