...
import { type EnvironmentId, ThreadId } from "@ito/contracts";
...
describe("rightPanelStore", () => {
  ...
  it("tracks one surface per pull request", () => {
    const first = { projectId: "project-a", repository: "pingdotgg/ito", number: 4909 };
    const second = { projectId: "project-a", repository: "pingdotgg/ito", number: 4910 };
    useRightPanelStore.getState().openPullRequest(refA, first);
    useRightPanelStore.getState().openPullRequest(refA, second);
    const url = "https://gitlab.example.com/pingdotgg/ito/-/merge_requests/4909";
