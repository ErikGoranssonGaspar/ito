...
`agent-device` command line, which Itô sets up for them.
...
Studio's Device Manager. Itô detects standard SDK locations; set
...
timeline. Agents drive the device through the `agent-device` command line. Itô
...
local network, Tailscale, and Itô. Live video needs a secure page
...
Itô checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its Itô helpers when reachable; simulators keep running.

Itô provides discovery, streaming, and control. Arrange app builds,
