...
Itô shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Itô directly from a
...
- **Itô** connects computers that are signed in to your account.
  [Install the CLI](./install.md#command-line) and run `ito connect` on each
  computer you want to add, then start Itô or run `ito serve` so the
  computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `ito serve`, then run `ito pair --tailscale` and paste
  the pairing link. You can also run `ito serve --host <address>` and use
  `ito pair` when the server is already reachable on your network.

Saved computers and computers discovered through Itô are selected by
...
If Itô cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Itô cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Itô checks each selected computer for Claude Code and Codex. If an agent is
...
Itô finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Itô keeps the
...
30 days. You can continue those conversations in Itô.

Conversation import is best effort. Itô keeps the first user prompt and the
...
conversation so Itô does not remove user text. It reads one conversation at
