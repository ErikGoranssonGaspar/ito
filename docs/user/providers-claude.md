...
Itô uses Claude Code's login and configuration. Start with the default provider
...
Itô uses the Claude configuration on the connected server.
...
Select the model you want in Itô. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Itô. Custom models may have
