...
Shift keys together. Itô attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Itô is in front captures Itô itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Itô refuses shortcuts
...
terminal contents. If an app is slow to answer, Itô attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Itô SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Itô under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Itô shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Itô shows the exact binding it will
...
**Other Wayland desktops.** Itô uses the desktop's screenshot portal when it can capture the
