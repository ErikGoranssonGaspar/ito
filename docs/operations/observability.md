...
> For maintainers. Using Itô? See [docs/user](../user/).

Itô has one server-side observability model:
...
stdout/stderr at `~/.ito/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.ito/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.ito/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.ito/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.ito/dev/logs/server.trace.ndjson`.
...
```bash
export ITO_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export ITO_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export ITO_OTLP_SERVICE_NAME=ito-local
```

Optional:

```bash
export ITO_TRACE_MIN_LEVEL=Info
export ITO_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `ITO_OTLP_*`.

macOS app bundle example:

```bash
ITO_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
ITO_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
ITO_OTLP_SERVICE_NAME=ito-desktop \
"/Applications/Itô.app/Contents/MacOS/Itô"
```
...
```bash
TRACE_FILE="${ITO_HOME:-$HOME/.ito}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.ito/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.ito/dev/logs/server.trace.ndjson"
```
...
- service name such as `ito-local`, `ito-dev`, or `ito-desktop`
...
- `ito_rpc_request_duration`
- `ito_orchestration_command_duration`
- `ito_orchestration_command_ack_duration`
- `ito_provider_turn_duration`
- `ito_git_command_duration`

Counters tell you volume and failure rate:

- `ito_rpc_requests_total`
- `ito_orchestration_commands_total`
- `ito_provider_turns_total`
- `ito_git_commands_total`
...
`ito_orchestration_command_ack_duration` measures:
...
1. Check `ito_orchestration_command_ack_duration` by `commandType`.
...
- `ITO_OTLP_TRACES_URL` was not set
...
- `ITO_TRACE_FILE`: override trace file path
- `ITO_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `ITO_TRACE_MAX_FILES`: rotated file count, default `10`
- `ITO_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `ITO_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `ITO_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `ITO_OTLP_TRACES_URL`: OTLP trace endpoint
- `ITO_OTLP_METRICS_URL`: OTLP metric endpoint
- `ITO_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `ITO_OTLP_SERVICE_NAME`: service name, default `ito-server`
- `ITO_OTLP_HEADERS`: extra headers for both exporters, same format as
  `OTEL_EXPORTER_OTLP_HEADERS`: comma-separated `key=value` pairs with percent-encoded values.
- `ITO_OTLP_PROTOCOL`: `http/json` (default) or `http/protobuf`
