...
Itô-managed OpenCode chat uses one server per thread. Its MCP registrations are directory-scoped, while
Itô's MCP connection is thread-scoped. Sharing a chat server between threads in one directory would
...
Itô auth session. The client carries the return URL back to the environment because the provider's
