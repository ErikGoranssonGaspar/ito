...
```text
[label](ito-context://v1/<kind>/<contextId>)
![label](ito-context://v1/image/<contextId>)
```

The parser accepts exactly the `ito-context:` scheme, the `v1` host, one kind segment matching
...
2. A trailing `<ito_context version="1">` envelope holds one `<context kind id>` entry per unique
...
3. Captured text is data: any `<` that would open or close `ito_context` or `context` is escaped,
...
runtime handler registry. The contract and codecs are shared, and the renderer draws rich chips.
Add a registry only when a third-party or runtime-defined kind
...
is, older messages are upgraded in memory. `ChatMarkdown` renders `ito-context://` links through
...
chips, a structured fragment under `web application/x-ito-context-fragment+json`
